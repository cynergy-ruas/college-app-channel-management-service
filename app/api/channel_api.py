...
from app.services.controller import (fetch_channels, create_channel, remove_channel,
                                      fetch_user_membership, fetch_channel, 
                                    user_leave, update_channel, join_user)
                                    # V1 add_admin,
...
async def add_user_to_channel( id: str, user_data: dict ):
    ...
    Raises:
        HTTPException: ["channel Not Found" or
...
@channelsRouter.post('/channels/leave/{id}')
...
# @channelsRouter.post('/channels/admin/{id}/{user_id}')
